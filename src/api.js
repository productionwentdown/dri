...
async function paginatable(path, scope, n, last = null) {
  ...
  if (response.headers.has('Link')) {
    const link = ParseLinkHeader.parse(response.headers.get('Link'))
    if (link && link.has('rel', 'next')) {
      nextLast = link.get('rel', 'next')
    }
  }
...
async function tags(name, last = null) {
  if (await usePortusExplore()) {
    const p = await portus()
    return {
      tags: p.find((r) => r.full_name === name).tags.map((t) => t[0].name),
    }
  }
...
async function tagCanDelete(name, ref) {
  ...
  try {
    ...
    request(
      'HEAD',
      `/v2/${name}/manifests/${headers.get('Docker-Content-Digest')}`,
      `repository:${name}:delete`,
    )
    return true
  } catch {
...
async function tagDelete(name, ref) {
  ...
  return request(
    'DELETE',
    `/v2/${name}/manifests/${tagManifest.headers.get('Docker-Content-Digest')}`,
    `repository:${name}:delete`,
  )
}
...
export { repos, tags, tag, tagCanDelete, tagDelete, repoCanDelete, repoDelete, blob, configBlob }
